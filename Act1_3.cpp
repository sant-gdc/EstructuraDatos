...
string diaCorrecto(vector<string> registro){
...
int fechaAgrupada(vector<string> registro){  
    string mes, dia, hora;
    mes = numMes(registro);
    dia = diaCorrecto(registro);
...
int bBinaria(vector<vector<string>> bitacora, int fecha){
    int inicio = 0;
    ...
    while(inicio <= fin){
        ...
        if(fecha == fechaAgrupada(bitacora[mitad])) {
			return mitad;
		} else if(fecha < fechaAgrupada(bitacora[mitad])){
            fin = mitad-1;
        }else{
            inicio = mitad + 1;
        }
