...
void DLinkedList::insertLast(string data){ //Inserta un nuevo nodo al final de la lista
    MyNodeLL* end = this->tail;
    this->tail = new MyNodeLL(data, nullptr, end);
    if(this->size==0){
        this->head = this->tail;
    }else{
        end->next = this->tail;
...
int DLinkedList::part(int ini, int fin){ //Particion de pivote en quicksort
    long long pivot = obtainIP(getAt(ini)->data);
    int x = ini + 1;

    for(int y = x; y <= fin; y++){
        long long IPtemp = obtainIP(getAt(y)->data);
        if(IPtemp < pivot){
